    ...
    def train(self, net):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                loss += 0.5 * 0.001 * torch.norm(torch.cat([x.view(-1) for x in net.parameters()]) - torch.cat([x.view(-1) for x in global_net.parameters()]), 2)
...
parser.add_argument('--fl_method', type=str, default='fedprox', help="name of federated learning method")
...
for iter in range(args.rounds):
    ...
    for idx in idxs_clients:
        local = LocalUpdate_fedprox(args, dataset=clients_total[idx])
        w, loss = local.train(net=copy.deepcopy(net_glob).to(args.device))

        w_locals.append(copy.deepcopy(w))
        loss_locals.append(copy.deepcopy(loss))

    # update global weights by average
